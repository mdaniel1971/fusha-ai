...
export default function ProgressClient() {
  ...
  const fetchProgress = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/observations/log-grammar?userId=${userId}&limit=100`
      );
      ...
      for (const obs of observations) {
        ...
        stat[
          obs.performance_level as keyof Pick<
            GrammarStat,
  ...
  if (loading) {
    return (
      ...
      >
        <p style={{ fontFamily: 'Arial, sans-serif', color: '#666' }}>
          Loading progress...
        </p>
      </div>
    );
  ...
  if (error) {
    return (
      ...
      >
        <p style={{ fontFamily: 'Arial, sans-serif', color: '#c00' }}>
          {error}
        </p>
        <button
  ...
  const totalObservations = grammarStats.reduce(
    (sum, s) => sum + s.mastered + s.emerging + s.struggling,
    0
  );
  const totalMastered = grammarStats.reduce((sum, s) => sum + s.mastered, 0);
  const overallMastery =
    totalObservations > 0
      ? Math.round((totalMastered / totalObservations) * 100)
      : 0;
                ...
                {recentObservations.map((obs) => (
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        {formatFeature(obs.grammar_feature)}:{' '}
                        {obs.grammar_value.replace(/_/g, ' ')}
                      </div>
                      {obs.student_attempt && (
                        <div
                          style={{
                        ...
                        >
                          {obs.student_attempt}
                          {obs.correct_form &&
                            obs.student_attempt !== obs.correct_form && (
                              <span
                                style={{
                                  color: '#22c55e',
                                  marginLeft: '0.5rem',
